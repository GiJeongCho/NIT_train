...
추출한 프레임은 기본적으로 **640×480** 으로 정규화해 저장한다
(`NIT_TRAIN_FRAME_RESIZE=0` 으로 끌 수 있다).

운영 추론(tracker_py)이 입력을 640×480으로 정규화해 처리하기 때문이다. 학습 이미지가 다른
해상도면 객체의 픽셀 크기 분포가 어긋나 특히 작은 표적의 성능이 떨어진다.
`imgsz=640` 학습과도 맞는다.
